...
class RedeemGiftViewController: UIViewController, Subscriber {
    ...
    private let titleLabel = UILabel(font: Fonts.Title.three, color: Colors.Contrast.two)
    private let separator = UIView(color: .white)
    private let icon = UIImageView()
    private let body = UILabel(font: Fonts.Body.one, color: Colors.Contrast.two)
    private let redeem = BRDButton(title: "Redeem", type: .secondary)
    private let statusCircle = LinkStatusCircle(colour: Colors.Contrast.two)
